...
void Chunk::updateEdgeCases(Chunk* otherChunk, Face Side)
{
	...
	case RIGHT_SIDE:
		...
		{
			for (int y = 0; y < CHUNK_HEIGHT; y++)
				chunkMap[get1DIndex(CHUNK_WIDTH - 1, y, z)]->updateFace(RIGHT_SIDE, otherChunk->getBlock(0, y, z)->info->isAir);
		}
	case BACK_SIDE:
		for (int x = 0; x < CHUNK_DEPTH; x++)
		{
			for (int y = 0; y < CHUNK_HEIGHT; y++)
				chunkMap[get1DIndex(x, y, 0)]->updateFace(BACK_SIDE, otherChunk->getBlock(x, y, CHUNK_DEPTH - 1)->info->isAir);
		}
	case FRONT_SIDE:
		for (int x = 0; x < CHUNK_DEPTH; x++)
		{
			for (int y = 0; y < CHUNK_HEIGHT; y++)
				chunkMap[get1DIndex(x, y, CHUNK_DEPTH - 1)]->updateFace(FRONT_SIDE, otherChunk->getBlock(x, y, 0)->info->isAir);
		}
	}
}